...
class RectangularRoom(object):
    ...
    """
    def __init__(self, width:int, height:int, dirt_amount:int):
        """
        ...
        self.width = width
        self.height = height
        self.tiles = {(i, j): dirt_amount for i in range(self.width) for j in range(self.height)}

        if self.width <= 0 or self.height <= 0:
            raise ValueError('Some values of the room are incorrect, please verify')
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        return len([i for i in self.tiles.values() if i == 0])
    ...
    def is_position_in_room(self, pos):
        ...
        """
        if self.width > math.floor(pos.get_x()) >= 0 and self.height > math.floor(pos.get_y()) >= 0:
            return True
        return False
...
class FurnishedRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        """
        unfurnished_tiles = [tile for tile in self.tiles if tile not in self.furniture_tiles]
        res = random.choice(unfurnished_tiles)
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        new_direction = float(random.randint(0, 360))

        # print(new_position)
        # print(new_direction)

        if self.room.is_position_valid(new_position) and self.position != new_position:
            self.set_robot_position(new_position)
            # print(self.position)
            self.room.clean_tile_at_position(self.position, self.capacity)
        else:
            while new_direction is not self.direction:
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for trials in range(num_trials):
        ...
        robots = [robot_type(room, speed, capacity) for robot in range(num_robots)]
        steps = 0
        min_cleaned_tiles = min_coverage * room.get_num_tiles()
        coverage = room.get_num_cleaned_tiles()
        anim = ps3_visualize.RobotVisualization(1, 5, 5, [])

        while coverage < min_cleaned_tiles:
            steps += 1
            for robot in robots:
                robot.update_position_and_clean()
                coverage = float(room.get_num_cleaned_tiles())
                anim.update(room, robots)      # See Robots working

        anim.done()
        total_trials_steps.append(steps)
    return np.average(total_trials_steps)



# print ('avg time steps: ' + str(run_simulation(1, 1.0, 1, 5, 5, 3, 1, 50, StandardRobot)))
